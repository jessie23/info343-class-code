...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function showNotification(title, body) {
        ...
        function dismissAlert() {
            note.close();   //closes notification after 3 seconds
        }

        window.setTimeout(dismissAlert, 3000); //in milliseconds
    }

    var triggerBtn = document.getElementById('trigger');
    triggerBtn.addEventListener('click', function() {
       switch (Notification.permission) { //like if/else if/else statement
           case 'granted':
